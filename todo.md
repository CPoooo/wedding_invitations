# Todoooooo
- get er done

# Misc
- get real collage images (looks like urweddingbestie examples)
- move images closer and overlap more
- add better typography to get same aesthetic
- initial envelope should look like a real envelope
- better font on front page (much more elegant)

# More components to add
- our story component
- frequently asked questions component
- more details component? (if different then FAQ)
- song component that is a record? 
- countdown component that is live and counting down 

# RSVP Page
- RSVP page aesthetics and form needs to be completed
- test functionality of this page and see if any other fields need to be addeed

# UI NEEDS TO BE PERFECT (elegant/old money/wedding vibes)
- Entire makeover of the ui until Rachel's approval (old money/wedding vibe aesthetic)
